...
function computeFilterId(csp, mask, filter, hostname, domains, denyallow, redirect) {
    ...
    if (csp !== undefined) {
        for (let i = 0; i < csp.length; i += 1) {
            hash = (hash * utils_js_1.HASH_INTERNAL_MULT) ^ csp.charCodeAt(i);
        }
    }
    ...
    if (redirect !== undefined) {
        for (let i = 0; i < redirect.length; i += 1) {
            hash = (hash * utils_js_1.HASH_INTERNAL_MULT) ^ redirect.charCodeAt(i);
        }
    }
    return hash >>> 0;
}
...
class NetworkFilter {
    static parse(line, debug = false) {
        ...
        if (optionsIndex !== -1 && line.charCodeAt(optionsIndex + 1) !== 47 /* '/' */) {
            ...
            for (const rawOption of line.slice(optionsIndex + 1).split(',')) {
                ...
                switch (option) {
                    case 'denyallow': {
                        denyallow = domains_js_1.Domains.parse(optionValue.split('|'), debug);
                        break;
                    }
                    case 'domain':
                    case 'from': {
                        ...
                        domains = domains_js_1.Domains.parse(optionValue.split('|'), debug);
                        break;
                    }
                    case 'badfilter':
    ...
    static deserialize(buffer) {
        ...
        return new NetworkFilter({
            ...
            domains: (optionalParts & 8) === 8 ? domains_js_1.Domains.deserialize(buffer) : undefined,
            ...
            denyallow: (optionalParts & 64) === 64 ? domains_js_1.Domains.deserialize(buffer) : undefined,
            regex: undefined,
        });
    }
    ...
    getSerializedSize(compression) {
        ...
        if (this.csp !== undefined) {
            estimate += (0, data_view_js_1.sizeOfNetworkCSP)(this.csp, compression);
        }
        if (this.filter !== undefined) {
        ...
        if (this.redirect !== undefined) {
            estimate += (0, data_view_js_1.sizeOfNetworkRedirect)(this.redirect, compression);
        }
        if (this.denyallow !== undefined) {
